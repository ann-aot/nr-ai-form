...
- STRICT: For any field with `type` of `radio`, the `suggestedvalue` MUST be one of the exact values from that field's `enum` list in the form context. Never use a value that is not in the enum list.
